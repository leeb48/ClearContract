...
import { businessDetailsSchema, changePasswordSchema, loginSchema, profileSchema, registerSchema } from './settings';
...
describe('profileSchema', () => {
	it('allows empty phone but not empty name', () => {
		expect(profileSchema.safeParse({ name: 'James', phone: '' }).success).toBe(true);
		expect(profileSchema.safeParse({ name: '', phone: '555' }).success).toBe(false);
	});
});
