...
export async function setup(): Promise<void> {
	...
	if (occupied) {
		throw new Error(`${PB_TEST_URL} is already serving — stop that instance before running integration tests`);
	}
	...
	const upsert = spawnSync(pbBin, ['superuser', 'upsert', PB_TEST_SUPERUSER, PB_TEST_SUPERUSER_PASS, ...common], {
		stdio: 'pipe',
	});
