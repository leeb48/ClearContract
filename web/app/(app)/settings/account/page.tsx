	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>Your name is used for the dashboard greeting.</CardDescription>
			</CardHeader>
			<CardContent>
				<form onSubmit={handleSubmit(onSubmit)} className="grid gap-4" noValidate>
					...
					<div className="grid gap-2">
						...
						<p className="text-xs text-zinc-500">Email changes arrive with email verification (Phase 4).</p>
					</div>
					<div className="grid gap-2">
...
function BusinessCard({ settings, onSaved }: { settings: RecordModel; onSaved: (r: RecordModel) => void }) {
	...
	const logoUrl = record.logo ? pb().files.getURL(record, record.logo, { thumb: '600x0' }) : null;
	...
	async function onSubmit(data: BusinessDetailsInput) {
		try {
			...
			const updated = await pb().collection('business_settings').update(record.id, form);
			onSaved(updated);
			setLogoFile(null);
			...
			<CardContent>
				<form onSubmit={handleSubmit(onSubmit)} className="grid gap-4" noValidate>
					<div className="grid gap-2">
						...
						<Input id="company-name" placeholder="James Driveways" {...field('company_name')} />
					...
					<div className="grid gap-2">
						...
						/>
						<p className="text-xs text-zinc-500">JPEG, PNG, WebP, or SVG, up to 5MB.</p>
					</div>
					<div>
						<Button type="submit" disabled={isSubmitting}>
			...
			<CardContent>
				<form onSubmit={handleSubmit(onSubmit)} className="grid gap-4" noValidate>
					...
					<div className="grid gap-2">
						...
						<Input id="new-password" type="password" autoComplete="new-password" {...field('password')} />
	...
	return (
		<Card className="border-red-200 dark:border-red-900">
			<CardHeader>
				...
				<CardDescription>
					Deleting your account removes your business settings and, in later phases, all quotes and customers.
			...
			<CardContent>
				{confirming ? (
					<div className="flex flex-wrap items-center gap-3">
						<span className="text-sm font-medium">Really delete your account?</span>
